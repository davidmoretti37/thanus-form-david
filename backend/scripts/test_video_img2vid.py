"""
Test script for image-to-video generation using Fal AI kling model.
"""
import asyncio
import os
import sys

# Add backend to path
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))

async def test_image_to_video():
    """Test image-to-video generation with kling model."""
    try:
        from fal_client import submit
        
        # Test parameters from user
        model = "fal-ai/kling-video/v2.5-turbo/pro/image-to-video"
        
        args = {
            "prompt": "A stark starting line divides two powerful cars, engines revving for the challenge ahead. They surge forward in the heat of competition, a blur of speed and chrome. The finish line looms as they vie for victory.",
            "image_url": "https://v3.fal.media/files/panda/HnY2yf-BbzlrVQxR-qP6m_9912d0932988453aadf3912fc1901f52.jpg",
            "duration": "5",
            "negative_prompt": "blur, distort, and low quality",
            "cfg_scale": 0.5
        }
        
        print(f"🎬 Testing image-to-video with model: {model}")
        print(f"📝 Prompt: {args['prompt'][:50]}...")
        print(f"🖼️  Image URL: {args['image_url']}")
        print(f"⏱️  Duration: {args['duration']}s")
        print("\n⏳ Submitting request to Fal AI...")
        
        # Submit to Fal AI
        handler = await asyncio.to_thread(submit, model, arguments=args)
        
        print("⏳ Waiting for video generation...")
        result = await asyncio.to_thread(handler.get)
        
        print("\n✅ Video generation complete!")
        print(f"📦 Full response: {result}")
        
        # Extract video URL
        def extract_url(obj):
            if isinstance(obj, dict):
                for k in ("url", "signed_url", "uri"):
                    v = obj.get(k)
                    if isinstance(v, str) and v.startswith("http"):
                        return v
                for v in obj.values():
                    found = extract_url(v)
                    if found:
                        return found
            elif isinstance(obj, list):
                for item in obj:
                    found = extract_url(item)
                    if found:
                        return found
            return None
        
        video_url = extract_url(result)
        if video_url:
            print(f"\n🎥 Video URL: {video_url}")
            print("\n✨ Success! The image-to-video model is working correctly.")
        else:
            print("\n❌ Could not extract video URL from response")
            
    except Exception as e:
        print(f"\n❌ Error: {str(e)}")
        import traceback
        traceback.print_exc()

if __name__ == "__main__":
    # Check for FAL_KEY
    if not os.getenv("FAL_KEY"):
        print("❌ FAL_KEY not found in environment")
        print("Please set FAL_KEY in backend/.env")
        sys.exit(1)
    
    print("="*60)
    print("🎬 Fal AI Image-to-Video Test (Kling Model)")
    print("="*60)
    print()
    
    asyncio.run(test_image_to_video())
