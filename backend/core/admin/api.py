...
async def admin_install_suna_for_user(
    ...
    logger.debug(f"Admin installing Tars agent for user: {account_id}")
    ...
    if agent_id:
        return {
            "success": True,
            "message": f"Successfully installed Tars agent for user {account_id}",
            "agent_id": agent_id
        }
    else:
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to install Tars agent for user {account_id}"
        )
