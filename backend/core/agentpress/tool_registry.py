...
class ToolRegistry:
    ...
    def clear_all_tools(self):
        ...
        This is useful when switching agent configurations and need to
        re-register tools based on new agent settings.
        """
        old_count = len(self.tools)
        self.tools.clear()
        logger.debug(f"Cleared {old_count} tools from registry")

    def get_registered_tool_names(self) -> List[str]:
        """Get list of all registered tool function names.

        Returns:
            List of tool function names currently registered
        """
        return list(self.tools.keys())

    def unregister_tool_functions(self, function_names: List[str]):
        """Unregister specific tool functions.

        Args:
            function_names: List of function names to remove from registry
        """
        removed_count = 0
        for func_name in function_names:
            if func_name in self.tools:
                del self.tools[func_name]
                removed_count += 1
                logger.debug(f"Unregistered tool function: {func_name}")

        logger.debug(f"Unregistered {removed_count} tool functions")

    def get_tool_statistics(self) -> Dict[str, Any]:
        """Get statistics about registered tools.

        Returns:
            Dict containing tool count and other statistics
        """
        tool_classes = set()
        for tool_info in self.tools.values():
            tool_classes.add(tool_info['instance'].__class__.__name__)

        return {
            "total_functions": len(self.tools),
            "unique_tool_classes": len(tool_classes),
            "tool_class_names": list(tool_classes),
            "function_names": list(self.tools.keys())
        }
